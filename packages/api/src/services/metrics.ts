 ...
 *
 * Two kinds live here, and they are not equally trustworthy in production:
 *
 *   1. **Counters and histograms** — incremented in-process. Production runs on
 *      Firebase App Hosting, which autoscales and discards instances, so each
 *      scrape reads one arbitrary instance's numbers and a scale-down throws
 *      them away. Directionally useful, never exact. Locally, where one `next
 *      dev` process serves everything, they are exact.
 *   2. **Gauges derived from the database** — recomputed on scrape from shared
 *      state, so they are correct no matter how many instances are running.
 *      These are the ones worth alerting on.
 *
 * Nothing here writes to Postgres. The database is a 0.5 GB Neon instance, and
 * a metrics system that grows it is a metrics system that takes the site down.
 * The collectors below are `count(*)` reads behind a 60-second cache, so a
 * 30-second scrape interval costs one round of counts a minute at most.
 */
import {
  Registry,
  Counter,
 ...
 * afterwards on purpose, so a failure here means money taken and nothing given
...
//
// Registered without a `collect` hook so they are never gathered implicitly:
// `refreshDbGauges` decides when a query is allowed to run, and the cache below
// is what keeps a scrape loop off the database.
...
/**
 * Answers "was the $15 plan worth adding". Read from what was charged, not from
...
/**
 * Paid, and attached to nobody. Every one of these is a person who was charged
 * and has no membership, so it should sit at zero and any climb is a bug in the
 * link paths rather than a busy day.
 */
...
class GaugeCache {
  ...
  /**
   * One refresh at a time, and at most one per TTL.
   *
   * Prometheus scrapes on a timer and a second scraper (or a page reload) can
   * land mid-flight; without the shared promise each one would start its own
   * round of counts against a database sized for a student club.
   */
 ...
 * module is pulled into the metrics route and the term rule is three lines.
 * If the two ever disagree the gauge is mislabelled, nothing more.
 */
...
/**
 * Recomputes the database-derived gauges, at most once a minute.
 *
 * Every query is a count behind an index-friendly predicate, and a failure only
 * leaves the previous values in place — a metrics endpoint must never be the
 * thing that takes a request path down.
 */
export async function refreshDbGauges(db: DrizzleDB | null | undefined) {
  ...
  await gaugeCache.refresh(async () => {
    try {
      ...
      /**
       * `like '{%'` before the jsonb cast, and deliberately so: a single row of
       * unparseable metadata would otherwise error the whole statement, and
       * rows written before the plan existed have no `plan` key at all — those
       * bought the only thing on offer at the time, a year.
       */
