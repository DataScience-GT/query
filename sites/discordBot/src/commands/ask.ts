...
class GuideSearchEngine {
  ...
  private async rebuildIndex(): Promise<void> {
    ...
    const searchData: SearchableSection[] = Object.entries(rawSections).map(
      ([title, lines]) => ({
        title,
        content: lines.join("\n"),
      }),
...
class SearchResultFormatter {
  ...
  static formatEmbed(result: SearchResult): EmbedBuilder {
    return new EmbedBuilder()
      ...
      .setFooter({
        text: `Match confidence: ${((1 - result.score) * 100).toFixed(0)}%`,
      });
  }

  static getWarningMessage(score: number): string | null {
    return score > 0.5
      ? "This might not be exactly what you're looking for, but it's the closest match I found:\n\n"
      : null;
  }
...
export async function execute(interaction: ChatInputCommandInteraction) {
  ...
  if (!rawQuery?.trim())
    return interaction.editReply("Please provide a question.");

  try {
    const results = await searchEngine.search(rawQuery);
    if (results.length === 0)
      return interaction.editReply({
        content: SearchResultFormatter.getNoResultsMessage(),
      });

    const topResult = results[0];
    const embed = SearchResultFormatter.formatEmbed(topResult);
    const warningMessage = SearchResultFormatter.getWarningMessage(
      topResult.score,
    );
    ...
    await interaction.editReply(
      "Search system temporarily offline. Please try again or use /guide to browse sections.",
    );
  }
}
