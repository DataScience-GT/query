...
class MockInteraction {
  ...
  get channel() {
    return this._createMockChannel("channel-current");
  }
  ...
  _extractContent(type: string, response: any) {
    ...
    else if (response?.embeds?.[0])
      text = `[Embed] ${response.embeds[0].data.title}`;
