...
const TrackCard: React.FC<{
  ...
  colorClass: string;
  num: string;
}> = ({ title, description, icon, colorClass, num }) => (
  <div
    className={`group relative w-full h-full glass-panel p-8 md:p-10 flex flex-col justify-between hover:-translate-y-2 transition-all duration-500 overflow-hidden`}
  >
    {/* Abstract Glow on Hover */}
    <div
      className={`absolute -bottom-20 -right-20 w-64 h-64 ${colorClass.replace("text-", "bg-")}/20 rounded-full blur-[80px] opacity-0 group-hover:opacity-100 transition-opacity duration-700 pointer-events-none`}
    ></div>

    <div className="flex justify-between items-start mb-12 relative z-10">
      <div
        className={`text-white group-hover:${colorClass} drop-shadow-md group-hover:drop-shadow-[0_0_15px_currentColor] transition-all duration-300`}
      >
        {icon}
      </div>
      <span
        className={`font-mono text-xs font-bold tracking-widest uppercase text-white/40 group-hover:${colorClass} transition-colors duration-300`}
      >
        Track {num}
      </span>
    </div>
    ...
    <div className="relative z-10">
      <h3
        className={`font-sans text-2xl md:text-3xl font-bold text-white mb-4 uppercase tracking-tighter group-hover:${colorClass} transition-colors duration-300`}
      >
        {title}
      </h3>
        ...
        <div className="w-full p-8 md:p-16 glass-panel relative overflow-hidden group">
          ...
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-8 relative z-10">
            <h1 className="font-sans text-5xl md:text-7xl lg:text-8xl font-bold tracking-tighter uppercase drop-shadow-lg">
              The
              <br />
              <span className="text-transparent bg-clip-text bg-gradient-to-r from-bloom-lime to-bloom-cyan bloom-text-glow">
                Tracks
              </span>
            </h1>
            ...
            <div className="md:w-1/3 bg-white/5 border border-white/10 rounded-2xl p-6 backdrop-blur-md shadow-xl group-hover:border-bloom-cyan/50 transition-colors duration-500">
              <p className="font-mono text-sm text-gray-200 uppercase tracking-widest leading-relaxed">
                Explore our{" "}
                <span className="text-bloom-pink font-bold drop-shadow-[0_0_5px_rgba(255,0,127,0.8)]">
                  5 digital tracks
                </span>
                . Compete to win overall or track-specific prizes.
              </p>
            </div>
          </div>
        </div>
