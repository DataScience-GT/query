...
function SponsorLogo({ name, logo, size = "md" }: { name: string; logo: string; size?: "lg" | "md" | "sm" | "xs" }) {
  ...
  return (
    <div className={`flex items-center justify-center border border-white/5 rounded-2xl bg-white/[0.02] backdrop-blur-3xl hover:bg-white/[0.04] hover:border-white/20 transition-all duration-500 ${dims.card}`}>
    ...
    <section id="sponsors" className="relative text-white overflow-hidden">
      <div className="max-w-7xl mx-auto px-6 py-32 md:py-48">
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-10 mb-24">
          ...
          <div className="flex flex-col items-start md:items-end gap-6 max-w-sm">
            ...
            <Link href="mailto:hello@hacklytics.io"
              className="inline-flex items-center justify-center px-6 py-3 text-sm font-medium text-white border border-white/10 rounded-full hover:bg-white/5 hover:border-white/20 transition-all duration-300 backdrop-blur-xl">
              Become a Partner
            </Link>
          </div>
        </div>
        ...
        <div className="space-y-16">
          <div>
            <span className="font-sans text-sm font-medium text-white/30 uppercase tracking-widest block mb-6">Gold</span>
            <div className="flex flex-wrap gap-4">
              {goldSponsors.map(s => <SponsorLogo key={s.name} {...s} size="lg" />)}
            </div>
          </div>
          <div>
            <span className="font-sans text-sm font-medium text-white/30 uppercase tracking-widest block mb-6">Silver</span>
            <div className="flex flex-wrap gap-4">
              {silverSponsors.map(s => <SponsorLogo key={s.name} {...s} size="md" />)}
            </div>
          </div>
          <div>
            <span className="font-sans text-sm font-medium text-white/30 uppercase tracking-widest block mb-6">Bronze</span>
            <div className="flex flex-wrap gap-4">
              {bronzeSponsors.map(s => <SponsorLogo key={s.name} {...s} size="sm" />)}
            </div>
          </div>
          <div>
            <span className="font-sans text-sm font-medium text-white/30 uppercase tracking-widest block mb-6">Additional</span>
            <div className="flex flex-wrap gap-4">
        ...
        <div className="mt-24 pt-8 border-t border-white/5 flex justify-end">
          <Link href="https://2025.hacklytics.io/#sponsors" target="_blank" rel="noopener noreferrer"
            className="font-sans text-sm text-white/30 hover:text-white transition-colors duration-300">
            View 2025 sponsors →
          </Link>
        </div>
