...
import { BLOOM, BUD } from "../pixel/sprites";
...
const FAQItem: React.FC<{ q: string; a: React.ReactNode; num: string }> = ({ q, a, num }) => {
  ...
  return (
    <div className={`border-b border-white/5 transition-colors duration-300 ${open ? "border-bloom-purple/40 bg-white/[0.02]" : "hover:bg-white/[0.02]"}`}>
      <button
      ...
      >
        <div className="flex items-start gap-6 md:gap-10">
          <PixelBullet n={Number(num)} tint="purple" />
          <span className={`font-sans text-lg md:text-2xl font-light tracking-wide transition-colors duration-300 ${open ? "text-white" : "text-white/70 group-hover:text-white"}`}>
            {q}
          </span>
        </div>
        {/* Pixel sprout that blooms when the answer opens */}
        <div className="shrink-0 mt-1">
          <PixelSprite
            ...
            scale={open ? 3 : 2}
            glow={open}
            className={open ? "animate-bob" : "opacity-50 group-hover:opacity-100"}
          />
        </div>
      </button>
  ...
  return (
    <section id="faqs" className="section-anchor text-white relative">
      <div className="section-wrap max-w-5xl mx-auto py-24 md:py-32 px-6">
        ...
        {/* Header */}
        <PixelLabel text="Information" tint="purple" />

        <div className="flex flex-col md:flex-row md:items-end md:justify-between gap-8 mb-20">
          <h2
            data-text="FAQ"
            className="neo-split-sm neo-glitch font-sans font-medium text-5xl sm:text-6xl md:text-7xl lg:text-[6rem] text-white leading-[0.9] tracking-[-0.03em]"
          >
            FAQ
          </h2>
