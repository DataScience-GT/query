...
const TrackPrizeCard: React.FC<{
  ...
  <div className={`glass-panel hover:bg-white/[0.05] transition-all duration-500 h-full flex flex-col group relative overflow-hidden hover:-translate-y-1 hover:border-${colorClass.replace('text-', '')}/50`}>
    ...
    <div className={`absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-64 h-64 ${colorClass.replace('text-', 'bg-')}/10 rounded-full blur-[60px] opacity-0 group-hover:opacity-100 transition-opacity duration-700 pointer-events-none`}></div>
    
    {/* Track Header */}
    <div className="border-b border-white/10 p-6 relative z-10">
      <h3 className={`font-sans text-2xl md:text-3xl font-bold uppercase tracking-tighter ${colorClass} mb-2 drop-shadow-[0_0_5px_currentColor]`}>
        {trackName}
      </h3>
...
const SpeakerCard: React.FC<{ name: string; title: string; company: string; image: string; colorClass: string }> = ({
  ...
  <div className={`glass-panel p-6 flex flex-col items-center text-center group hover:-translate-y-2 transition-all duration-500 relative overflow-hidden hover:border-${colorClass.replace('text-', '')}/50`}>
    ...
    {/* Info */}
    <h3 className={`font-sans text-2xl font-bold uppercase tracking-tight text-white mb-2 group-hover:${colorClass} transition-colors drop-shadow-md`}>
      {name}
    </h3>
