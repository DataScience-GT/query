    ...
    return (
        <LiquidGlass className="rounded-none overflow-hidden mb-12">
            <div className="p-6 border-b border-[var(--border-subtle)]">
                ...
                <p className="text-xs text-[var(--text-subtle)] font-mono mt-1">{rankings.rankings.length} projects assigned</p>
            </div>
                ...
                <table className="w-full">
                    ...
                    <tbody>
                        {[...rankings.rankings]
                            ...
                            .map((r) => (
                                <tr key={r.project.id} className="border-b border-[var(--border-subtle)] hover:bg-white/[0.02] transition-colors">
                                    ...
                                    <td className="py-4 px-6">
                                        <p className="text-sm font-bold text-[var(--text-primary)]">{r.project.name}</p>
                                    </td>
                                    <td className="py-4 px-6">
                                        <p className="text-xs text-[var(--text-subtle)] font-mono">{r.project.teamMembers || '-'}</p>
                                    </td>
                                    <td className="py-4 px-6">
                                        {r.project.tracks?.[0] ? (
                                    ...
                                    <td className="py-4 px-6">
                                        <div className="flex flex-wrap gap-1">
                                            {(r.project.tracks?.slice(1) || []).concat(r.project.challenges || []).map((t: string, i: number) => (
                                                <span key={i} className="px-2 py-0.5 rounded-sm text-[8px] font-bold bg-white/5 text-[var(--text-muted)] border border-[var(--border-subtle)] uppercase tracking-widest">
                                                    {t}
                                                </span>
                                            ))}
                                            {r.project.isCreateX && (
                                    ...
                                    <td className="py-4 px-6">
                                        <span className="text-sm font-bold text-[var(--text-muted)] tabular-nums">{r.voteCount}</span>
                                    </td>
                                    <td className="py-4 px-6">
                                        <span className="text-lg font-black text-[var(--text-primary)] tabular-nums">{r.avgScore}</span>
                                        <span className="text-xs text-gray-600 ml-1">/50</span>
                                    </td>
                                    <td className="py-4 px-6">
                                        <div className="flex items-center gap-2">
                                            <span className="text-lg font-black text-accent tabular-nums">{r.weightedScore}</span>
