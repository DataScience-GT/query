        ...
        >
          <div className="flex-1 relative flex flex-col bg-[#0a0a0a] rounded-none p-6 md:p-8 overflow-hidden z-10">
            ...
            <div className="flex items-start justify-between mb-4 relative z-10">
              <h3 className="text-xl font-bold text-[var(--text-primary)] group-hover:text-emerald-300 transition-colors leading-tight">
                {project.name}
              </h3>
              {project.status === "winner" && (
                <span className="bg-amber-400/10 text-amber-400 border border-amber-400/20 text-[10px] font-bold uppercase tracking-widest px-2.5 py-1 rounded-sm shadow-[0_0_15px_rgba(251,191,36,0.3)] shrink-0 ml-3">
                  Winner
                </span>
              )}
            </div>

            <p className="text-sm text-[var(--text-primary)]/50 mb-6 line-clamp-3 leading-relaxed relative z-10 flex-1">
              {project.description}
            </p>
              ...
              {project.technologies && project.technologies.length > 0 && (
                <div className="flex flex-wrap gap-2 mb-4">
                  {project.technologies.slice(0, 4).map((tech) => (
                    <span
                      key={tech}
                      className="text-[10px] font-semibold text-[var(--text-primary)]/40 bg-white/5 border border-[var(--border-subtle)] px-2.5 py-1 rounded-md uppercase tracking-wider"
                    >
                      {tech}
                    </span>
                  ))}
                  {project.technologies.length > 4 && (
                    <span className="text-[10px] font-semibold text-[var(--text-primary)]/30 px-1 py-1">
                      +{project.technologies.length - 4}
                    </span>
                  )}
                </div>
              ...
              {project.challenges && project.challenges.length > 0 && (
                <div className="mb-5 space-y-1.5">
                  {project.challenges.map((c) => (
                    <div
                      key={c}
                      className="text-xs font-medium text-accent/80 flex items-center gap-2"
                    >
