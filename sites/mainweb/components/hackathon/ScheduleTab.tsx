...
export function ScheduleTab({ hackathonId, isRegistered }: { hackathonId: string; isRegistered: boolean }) {
    ...
    if (eventsLoading) return <div className="py-16 text-center text-xs font-semibold uppercase tracking-widest text-[var(--text-primary)]/40 animate-pulse">Loading schedule...</div>;
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            <div className="lg:col-span-1 space-y-6">
                <LiquidGlass className="p-8 text-center flex flex-col items-center justify-center relative overflow-hidden bg-white/[0.01] border-[var(--border-subtle)]">
                    ...
                    <h3 className="text-xl font-bold text-[var(--text-primary)] tracking-tight mb-2">My Event Pass</h3>
                    <p className="text-xs text-[var(--text-primary)]/40 mb-8 leading-relaxed">Present this pass to check into workshops, meals, and other events.</p>
                ...
                {isRegistered && myRecord && (
                    <LiquidGlass className="p-6 bg-white/[0.01] border-[var(--border-subtle)]">
                        <h4 className="text-[11px] font-bold text-[var(--text-primary)]/40 uppercase tracking-widest mb-5">Registration Details</h4>
                        <ul className="space-y-4 text-sm text-[var(--text-primary)]/60">
                            <li className="flex justify-between items-center bg-white/[0.02] p-3 rounded-none border border-[var(--border-subtle)]">
                                <span>Status</span>
                                <span className="text-accent font-semibold">{myRecord.registrationStatus}</span>
                            </li>
                            <li className="flex justify-between items-center bg-white/[0.02] p-3 rounded-none border border-[var(--border-subtle)]">
                                <span>Shirt Size</span>
                                <span className="text-[var(--text-primary)] font-medium">{myRecord.shirtSize || 'Pending'}</span>
                            </li>
                        </ul>
                    </LiquidGlass>
                )}
            ...
            <div className="lg:col-span-2 space-y-4">
                <LiquidGlass className="p-8 md:p-10 bg-white/[0.01] border-[var(--border-subtle)]">
                    <div className="flex items-center justify-between mb-10 pb-6 border-b border-[var(--border-subtle)]">
                        <h3 className="text-2xl font-bold text-[var(--text-primary)] tracking-tight">Schedule</h3>
                    ...
                    {!events || events.length === 0 ? (
                        <div className="py-16 flex flex-col items-center justify-center text-center">
                            <div className="w-16 h-16 bg-white/[0.02] border border-[var(--border-subtle)] rounded-none flex items-center justify-center mb-6">
                                <svg className="w-8 h-8 text-[var(--text-primary)]/30" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z" /></svg>
                            </div>
                            <h3 className="text-lg font-medium text-[var(--text-primary)] mb-2">No Events Scheduled</h3>
                            <p className="text-sm text-[var(--text-primary)]/40">Check back later for updates to the event itinerary.</p>
                        </div>
                    ) : (
                            ...
                            {events.map((event) => (
                                <div key={event.id} className="relative pl-8 group">
                                    ...
                                    <div className="flex flex-col md:flex-row md:items-start justify-between gap-3 mb-3">
                                        <h4 className="text-xl font-semibold text-[var(--text-primary)] group-hover:text-emerald-300 transition-colors">{event.name}</h4>
