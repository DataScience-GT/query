...
export function FormChipSelect({ label, required, options, value, onChange, allowDeselect = false }: FormChipSelectProps) {
    return (
        <div>
            <label className="block text-[11px] uppercase tracking-widest font-semibold text-[var(--text-primary)]/50 mb-2">
                {label}{required && ' *'}
...
export function FormMultiChipSelect({ label, options, selected, onChange, noneOption }: FormMultiChipSelectProps) {
    function toggle(opt: string) {
        if (opt === noneOption) { onChange([]); return; }
        onChange(selected.includes(opt) ? selected.filter(s => s !== opt) : [...selected, opt]);
    }
    return (
        <div>
            <label className="block text-[11px] uppercase tracking-widest font-semibold text-[var(--text-primary)]/50 mb-2">{label}</label>
            <div className="flex flex-wrap gap-2.5">
                {options.map(opt => (
                    <button
                        key={opt}
...
export function SearchableSelect({ label, required, placeholder, value, onChange, options, allowCustom = true }: SearchableSelectProps) {
            ...
            {steps.map((label, i) => (
                <div key={label} className="flex-1 flex flex-col items-center gap-2">
                    <div className="w-full flex items-center">
                        <div className={`flex-1 h-1 rounded-sm transition-all duration-500 ${i <= current ? 'bg-emerald-500' : 'bg-white/10'}`} />
                    </div>
                    <span className={`text-[10px] font-bold uppercase tracking-widest transition-colors ${i <= current ? 'text-accent' : 'text-[var(--text-primary)]/30'}`}>
                        {label}
                    </span>
                </div>
            ))}
...
export function StepContainer({ title, children }: { title: string; children: React.ReactNode }) {
    return (
        <div className="space-y-5 animate-in fade-in duration-300">
            <h4 className="text-lg font-bold text-[var(--text-primary)] tracking-tight">{title}</h4>
            {children}
        </div>
    );
}
...
export function FormNavigation({ step, totalSteps, onBack, onNext, onSubmit, onCancel, isSubmitting }: FormNavigationProps) {
    const isLastStep = step >= totalSteps - 1;
    return (
        <div className="flex flex-col sm:flex-row items-center gap-4 pt-8">
            {step > 0 && (
                <button onClick={onBack} type="button" className="w-full sm:w-auto px-6 py-4 rounded-none bg-white/5 border border-[var(--border-subtle)] text-[var(--text-primary)]/60 hover:text-[var(--text-primary)] hover:bg-white/10 font-bold text-sm uppercase tracking-widest transition-all">
                    Back
                </button>
            )}
            {!isLastStep ? (
                <button onClick={onNext} type="button" className="w-full sm:w-auto px-8 py-4 rounded-none bg-emerald-500 text-[#020202] font-bold text-sm uppercase tracking-widest hover:bg-emerald-400 transition-all duration-300 hover:shadow-[0_0_20px_rgba(16,185,129,0.4)] hover:-translate-y-0.5">
                    Continue
                </button>
            ) : (
                <button onClick={onSubmit} type="button" disabled={isSubmitting} className="w-full sm:w-auto px-8 py-4 rounded-none bg-emerald-500 text-[#020202] font-bold text-sm uppercase tracking-widest hover:bg-emerald-400 transition-all duration-300 disabled:opacity-50 hover:shadow-[0_0_20px_rgba(16,185,129,0.4)] hover:-translate-y-0.5">
                    {isSubmitting ? 'Submitting...' : 'Submit Application'}
                </button>
            )}
            <button onClick={onCancel} type="button" className="w-full sm:w-auto px-6 py-4 text-[var(--text-primary)]/40 hover:text-[var(--text-primary)] text-sm font-bold uppercase tracking-widest transition-colors">
                Cancel
            </button>
        </div>
    );
