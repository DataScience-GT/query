...
export function FormInput({ label, required, error, className = '', ...props }: FormInputProps) {
    return (
        <div>
            <label className="block text-[11px] uppercase tracking-widest font-semibold text-white/50 mb-2">
                {label}{required && ' *'}
            </label>
            <input
                {...props}
...
export function FormMultiChipSelect({ label, options, selected, onChange, noneOption }: FormMultiChipSelectProps) {
    ...
    return (
        <div>
            <label className="block text-[11px] uppercase tracking-widest font-semibold text-white/50 mb-2">{label}</label>
            <div className="flex flex-wrap gap-2.5">
                {options.map(opt => (
                    <button
                        key={opt}
                ...
                <div className="absolute z-50 mt-2 w-full max-h-48 overflow-y-auto bg-[#0e0e0e] border border-white/10 rounded-xl shadow-2xl backdrop-blur-xl">
                    {filtered.map(opt => (
                        <button
                            ...
                            className={`w-full text-left px-4 py-2.5 text-sm font-medium hover:bg-white/5 transition-colors ${value === opt ? 'text-cyan-400 bg-cyan-500/5' : 'text-white/70'}`}
                        >
                            {opt}
                        </button>
                    ))}
...
export function FormNavigation({ step, totalSteps, onBack, onNext, onSubmit, onCancel, isSubmitting }: FormNavigationProps) {
    ...
    return (
        <div className="flex flex-col sm:flex-row items-center gap-4 pt-8">
            ...
            <button onClick={onCancel} type="button" className="w-full sm:w-auto px-6 py-4 text-white/40 hover:text-white text-sm font-bold uppercase tracking-widest transition-colors">
                Cancel
            </button>
        </div>
    );
