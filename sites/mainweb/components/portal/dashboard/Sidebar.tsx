...
export function Sidebar({
    ...
    return (
        <LiquidGlass className="p-6 relative overflow-visible">
            {/* User Profile Header */}
            <div className="flex items-center gap-5 border-b border-[var(--border-subtle)] pb-8 mb-8">
                <div className="relative group">
                    <div className="absolute -inset-0.5 bg-gradient-to-r from-accent to-blue-600 rounded-sm opacity-50 blur group-hover:opacity-75 transition duration-1000 group-hover:duration-200" />
                    <Image
                ...
                <div className="space-y-1">
                    <p className="text-[var(--text-primary)] font-bold uppercase tracking-tight text-base font-mono">
                        {user.name || 'GUEST'}
                    </p>
                    <div className="flex items-center gap-2">
                        <div className={`h-2 w-2 rounded-sm ${statusConfig[status].color}`} />
                        <p className="text-[var(--text-subtle)] text-xs uppercase tracking-widest font-bold">
            ...
            <nav className="space-y-2">
              ...
              ${activeTab === tab.key
                                ...
                                : 'text-[var(--text-subtle)] hover:text-[var(--text-primary)] hover:bg-white/[0.02]'
                            }`}
                    >
                        <span>{tab.label}</span>
                        <span
                            className={`h-2 w-2 rounded-sm transition-all ${activeTab === tab.key ? 'bg-accent' : 'bg-transparent group-hover:bg-white/20'
                                }`}
                        />
                    </button>
                ))}
