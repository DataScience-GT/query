      ...
      <div className="relative z-10 flex-1">
        ...
        {accessLevel && (
          <div className="flex items-center gap-2 mb-3">
            ...
            <p
              className={`text-xs uppercase tracking-[0.2em] font-bold ${classes.accessColor}`}
            >
              {accessLevel}
            </p>
          </div>
        ...
        {/* Title */}
        <h3
          className={`text-2xl font-bold text-[var(--text-primary)] uppercase tracking-tight mb-2 ${classes.titleHover} transition-colors`}
        >
          {title}
        </h3>
        ...
        {description && (
          <p
            className={`text-sm ${variant === "member" ? "text-[var(--text-subtle)] font-mono group-hover:text-[var(--text-muted)] transition-colors" : "text-[var(--text-subtle)] font-mono"} mb-6 flex-1`}
          >
            {variant === "member" && "> "}
            {description}
          </p>
        ...
        {statusBadge && (
          <div
            className={`inline-flex items-center gap-2 px-3 py-1.5 rounded bg-${statusBadge.color}-500/10 border border-${statusBadge.color}-500/20 self-start mt-auto`}
          >
            <div
              className={`h-2 w-2 rounded-sm bg-${statusBadge.color}-500 animate-pulse`}
            />
            <span
              className={`text-[10px] font-mono text-${statusBadge.color}-500 font-bold uppercase tracking-wider`}
            >
              {statusBadge.label}
            </span>
          </div>
