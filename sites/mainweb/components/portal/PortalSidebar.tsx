...
export default function PortalSidebar({ isOpen, setIsOpen }: PortalSidebarProps) {
  ...
  return (
    <>
      ...
      {/* DESKTOP SIDEBAR */}
      <div className={`hidden md:flex flex-col fixed left-0 top-0 z-40 h-screen border-r border-[var(--border-subtle)] bg-[var(--bg-primary)] dark:bg-darkBlue/80 backdrop-blur-3xl transition-all duration-300 ${isOpen ? 'w-64' : 'w-20'}`}>
        ...
        <div className="border-t border-[var(--border-subtle)] px-3 py-4">
          ...
          {isOpen && (
            <div className="mt-3 flex flex-col gap-2 w-full">
              ...
              <button onClick={() => signOut({ callbackUrl: '/login' })} className="flex w-full items-center gap-3 rounded-none px-3 py-3 text-[var(--text-subtle)] hover:bg-red-500/10 hover:text-red-400 transition-colors">
