      ...
      <div className="relative z-10 max-w-6xl mx-auto py-24 px-6 md:px-12">
        ...
        <div className="mb-10">
          ...
          <p className="text-base text-[var(--text-primary)]/40 max-w-xl leading-relaxed">
            Browse upcoming competitions, manage your registrations, and
            ...
            {!hackathons || hackathons.length === 0 ? (
              <LiquidGlass className="relative z-10 max-w-lg w-full mx-auto p-12 text-center flex flex-col items-center border border-[var(--border-subtle)]">
                <div className="w-20 h-20 bg-white/[0.02] border border-[var(--border-subtle)] rounded-none flex items-center justify-center mb-6">
                  <svg
                    className="w-8 h-8 text-[var(--text-primary)]/40"
                    fill="none"
                    stroke="currentColor"
                    viewBox="0 0 24 24"
                  >
                ...
                <h2 className="text-xl text-[var(--text-primary)] font-medium mb-2">
                  No Active Hackathons
                </h2>
                <p className="text-sm text-[var(--text-primary)]/40 leading-relaxed">
                  There are currently no hackathons available. Check back soon.
                </p>
              </LiquidGlass>
            ) : (
                        ...
                        <LiquidGlass className="h-full flex flex-col p-1 transition-ui duration-500 hover:-translate-y-1 hover:shadow-[0_20px_40px_-15px_rgba(0,0,0,0.5)] bg-white/[0.01] border-[var(--border-subtle)] hover:bg-white/[0.02] hover:border-white/20">
                          <div className="relative flex flex-col h-full bg-[var(--bg-tertiary)] rounded-none p-6 md:p-8 overflow-hidden z-10">
                            ...
                            <div className="flex flex-wrap justify-between items-start gap-3 mb-8 relative z-10">
                              <div
                                className={`inline-flex items-center gap-2 px-3 py-1.5 rounded-sm ${conf.bg} border ${conf.border} backdrop-blur-md`}
                              >
                                <div
                                  className={`h-1.5 w-1.5 rounded-sm ${conf.dot} ${conf.glow} ${h.status === "open" || h.status === "in_progress" ? "animate-pulse" : ""}`}
                                />
                                <span
                            ...
                            <div className="flex-1 relative z-10">
                              {h.theme && (
                                <p className="text-accent/80 text-xs font-mono mb-3 uppercase tracking-wider">
                                  {h.theme}
                                </p>
                              )}
                              <h2 className="text-2xl font-bold text-[var(--text-primary)] mb-3 group-hover:text-emerald-300 transition-colors duration-300 leading-tight">
                                {h.name}
                              </h2>
                              {h.description && (
                                <p className="text-sm text-[var(--text-primary)]/50 line-clamp-3 leading-relaxed mb-6">
                                  {h.description}
                                </p>
                              )}
                            </div>
                            ...
                            <div className="mt-auto pt-6 border-t border-[var(--border-subtle)] flex flex-col gap-3 relative z-10">
                              ...
                              {h.maxParticipants && (
                                <div className="flex items-center gap-2.5 text-sm text-[var(--text-primary)]/60">
                                  ...
                                  <span className="font-medium">
                                    {h.currentParticipants} /{" "}
                                    {h.maxParticipants} Spots
                                  </span>
                                </div>
                              )}
                            </div>
                ...
                <div className="relative z-10 flex flex-col md:flex-row items-center justify-between gap-8">
                  <div className="text-center md:text-left">
                    ...
                    <p className="text-[var(--text-primary)]/60 max-w-xl leading-relaxed text-sm">
                      Help evaluate amazing projects, mentor emerging
                      developers, and be a core part of our hackathon community.
                      We're always looking for industry professionals and
                      experienced students to join our judging panels.
                    </p>
                  </div>
                  <Link
                    href="/judge/register"
            ...
            {myRegs && myRegs.length > 0 ? (
                ...
                {myRegs.map((reg) => (
                  ...
                  >
                    ...
                    <div className="flex justify-between items-start mb-6 relative z-10">
                      <span
                        className={`px-3 py-1.5 rounded-none text-[10px] font-mono uppercase tracking-widest font-bold shadow-lg ${
                          reg.registrationStatus === "approved"
                            ? "bg-accent/20 text-accent border border-accent/30 shadow-[var(--accent)]/20"
                            : reg.registrationStatus === "rejected"
                              ? "bg-red-500/20 text-red-500 border border-red-500/30 shadow-red-500/20"
                    ...
                    <p className="text-[var(--text-muted)] text-sm line-clamp-3 mb-6 flex-1 relative z-10 leading-relaxed">
              ...
              <div className="flex flex-col items-center justify-center p-16 md:p-24 text-center border border-[var(--border-subtle)] rounded-none bg-[var(--bg-primary)] shadow-2xl">
                ...
                <p className="text-[var(--text-muted)] mb-8 max-w-sm">
                  You haven't registered for any upcoming hackathons yet. Browse
            ...
            {projects.length === 0 ? (
              <div className="flex flex-col items-center justify-center p-16 md:p-24 text-center border border-[var(--border-subtle)] rounded-none bg-[var(--bg-primary)] shadow-2xl">
                ...
                <p className="text-[var(--text-muted)] max-w-sm mb-8">
                  Join a hackathon team and submit a project to see your
                  portfolio grow here.
                </p>
                <button
                  onClick={() => setActiveTab("browse")}
                  ...
                  >
                    ...
                    <div className="relative z-10">
                      <div className="flex justify-between items-start mb-6">
                        <h4 className="text-3xl font-black text-[var(--text-primary)] italic group-hover:text-accent transition-colors">
                          {project.name}
                        </h4>
                        <span
                          className={`px-3 py-1.5 rounded-none text-[10px] font-mono uppercase font-bold tracking-widest border shadow-lg ${
                            project.status === "winner"
                              ? "border-yellow-500/40 bg-yellow-500/20 text-yellow-500 shadow-yellow-500/20"
                              : project.status === "judging"
                                ? "border-blue-500/40 bg-blue-500/20 text-blue-500 shadow-blue-500/20"
