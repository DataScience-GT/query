        ...
        <div className="max-w-7xl mx-auto px-4 py-4">
          <div className="flex items-start justify-between gap-4">
            <div className="flex-1 min-w-0">
              <Link href="/admin/hackathons" className="flex items-center gap-2 text-text-muted hover:text-[var(--text-primary)] transition-colors text-xs font-mono uppercase tracking-wider group w-fit" aria-label="Back to hackathons hub">
                <div className="p-1 rounded-none bg-white/5 border border-[var(--border-subtle)] group-hover:bg-accent/20 group-hover:border-accent/40 transition-colors">
                  <svg className="w-4 h-4 text-[var(--text-primary)] group-hover:text-accent transition-colors" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M10 19l-7-7m0 0l7-7m-7 7h18" /></svg>
                </div>
                <span className="hidden md:inline text-[var(--text-primary)]/60 group-hover:text-[var(--text-primary)] transition-colors">Hackathons</span>
              </Link>
      ...
      <div className="md:hidden fixed bottom-0 left-0 right-0 bg-[var(--bg-primary)]/98 backdrop-blur-2xl border-t border-[var(--border-subtle)] z-40 grid grid-cols-5 pb-safe">
        ...
        {tabs.map((tab) => (
          ...
          >
            ...
            <span className={`text-[10px] font-bold uppercase tracking-wider transition-colors ${activeTab === tab.id ? 'text-accent' : 'text-text-muted group-hover:text-[var(--text-muted)]'}`}>
              {tab.label}
            </span>
