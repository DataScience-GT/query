      ...
      <div className="relative z-10 max-w-7xl mx-auto">
        ...
        <div className="space-y-6">
          ...
          <div className="flex items-center justify-between">
            ...
            <p className="text-[var(--text-subtle)] text-sm">{projects?.length || 0} total projects</p>
          </div>

          <div className="space-y-4">
            {!selectedHackathon ? (
              <LiquidGlass className="p-16 text-center">
                <h3 className="text-[var(--text-primary)] font-semibold mb-1">Select a hackathon</h3>
                <p className="text-[var(--text-subtle)] text-sm">Choose a hackathon above to view its submitted projects.</p>
              </LiquidGlass>
            ) : isLoading ? (
              ...
              <LiquidGlass className="p-16 text-center">
                ...
                <h3 className="text-[var(--text-primary)] font-semibold mb-1">No projects yet</h3>
                <p className="text-[var(--text-subtle)] text-sm">Hackathon projects will appear here once participants submit them.</p>
              </LiquidGlass>
            ) : (
              <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
                {projects.map((project) => (
                  <LiquidGlass key={project.id} className="p-5 hover:border-accent/30 transition-all group">
                    <div className="flex items-start justify-between mb-3">
                      <div className="flex-1">
                        <h3 className="text-lg font-bold text-[var(--text-primary)] group-hover:text-accent transition-colors">
                          {project.name}
                        </h3>
                        <p className="text-sm text-[var(--text-subtle)] mt-1 line-clamp-2">{project.description || 'No description'}</p>
                      </div>
                      <span className={`px-2 py-1 rounded-sm text-xs font-semibold ${getStatusColor(project.status)}`}>
                        {project.status}
                      </span>
                    </div>
