        ...
        <div className="lg:col-span-1 space-y-2">
          {[
            ...
            },
            {
              ...
              desc: "Authentication policies",
            },
            {
              id: "integrations",
              icon: Database,
              label: "Integrations",
              desc: "External API connections",
            },
          ].map((tab) => (
            <button
              key={tab.id}
              onClick={() =>
                setActiveTab(tab.id as "general" | "security" | "integrations")
              }
            ...
            >
              <div className="flex items-center gap-3 w-full">
                <tab.icon
                  className={`w-5 h-5 ${activeTab === tab.id ? "text-accent" : "text-[var(--text-muted)]"}`}
                />
                <span
                  className={`font-bold ${activeTab === tab.id ? "text-[var(--text-primary)]" : "text-gray-300"}`}
                >
                  {tab.label}
                </span>
              </div>
              <span className="text-xs text-[var(--text-subtle)] mt-2 font-mono ml-8">
                {tab.desc}
              </span>
            </button>
          ))}
                  ...
                  <div className="space-y-2">
                    ...
                    <input
                      ...
                      onChange={(e) =>
                        setSettings({
                          ...settings,
                          maxEventCapacity: parseInt(e.target.value) || 0,
                        })
                      }
                      className="w-full bg-[var(--bg-primary)]/50 border border-[var(--border-subtle)] rounded-none px-4 py-3 text-[var(--text-primary)] focus:outline-none focus:border-accent focus:ring-1 focus:ring-accent transition-all"
                    />
                  ...
                  <div className="flex items-center justify-between p-4 rounded-none bg-[var(--bg-primary)]/30 border border-[var(--border-subtle)]">
                    <div>
                      ...
                      <p className="text-sm text-[var(--text-subtle)] mt-1">
                        Disables access for non-admin users across the platform.
                      </p>
                    </div>
                  ...
                  <div className="flex items-center justify-between p-4 rounded-none bg-[var(--bg-primary)]/30 border border-[var(--border-subtle)] transition-colors hover:border-accent/30">
                    <div className="flex gap-4 items-start">
                      ...
                      <div>
                        ...
                        <p className="text-sm text-[var(--text-subtle)] mt-1">
                          Allow new users to sign up without an admin invite.
                        </p>
                      </div>
                    </div>
                    <label className="relative inline-flex items-center cursor-pointer">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div className="p-5 rounded-none bg-[var(--bg-primary)]/30 border border-purple-500/20 hover:border-purple-500/50 transition-all group">
                    <div className="flex justify-between items-start mb-4">
                      ...
                      <span className="px-2 py-1 text-[10px] font-bold uppercase tracking-wider text-green-400 bg-green-400/10 rounded-md">
                        Connected
                      </span>
                    </div>
                    <h4 className="text-[var(--text-primary)] font-bold mb-1">
                      Email Service
                    </h4>
                    <p className="text-xs text-[var(--text-subtle)] mb-4">
                      SMTP configuration for automated notifications and
                      invites.
                    </p>
                  ...
                  <div className="p-5 rounded-none bg-[var(--bg-primary)]/30 border border-[var(--border-subtle)] hover:border-white/20 transition-all group opacity-50 grayscale hover:grayscale-0 hover:opacity-100">
                    <div className="flex justify-between items-start mb-4">
                      ...
                      <span className="px-2 py-1 text-[10px] font-bold uppercase tracking-wider text-[var(--text-muted)] bg-gray-400/10 rounded-md">
                        Not Configured
                      </span>
                    </div>
                    <h4 className="text-[var(--text-primary)] font-bold mb-1">
                    ...
                    <p className="text-xs text-[var(--text-subtle)] mb-4">
                      Sync roles and announce events to Discord server.
                    </p>
