...
export default function JudgePage() {
  ...
  const { data: hackathons, isLoading: hackathonsLoading } = trpc.hackathon.list.useQuery({});
  const { data: assignments } = trpc.judge.getMyAssignments.useQuery(undefined, { enabled: !!session && !!judgeStatus?.isJudge });
  ...
  const confMap: Record<string, { label: string; bg: string; border: string }> = {
    open: { label: 'Accepting Judges', bg: 'bg-accent/10', border: 'border-accent/30' },
    in_progress: { label: 'Active', bg: 'bg-accent/10', border: 'border-accent/30' },
    completed: { label: 'Completed', bg: 'bg-white/5', border: 'border-[var(--border-subtle)]' },
    closed: { label: 'Closed', bg: 'bg-amber-400/10', border: 'border-amber-400/30' },
      ...
      <div className="relative z-10 max-w-7xl mx-auto py-16 px-6 md:px-12">
        <div className="flex flex-col md:flex-row md:items-end justify-between mb-12 gap-6">
          <div className="max-w-xl">
            <Link href="/dashboard" className="inline-flex items-center gap-2 text-text-muted hover:text-[var(--text-primary)] transition-colors text-sm font-medium mb-6 group">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {hackathons?.map((h: HackathonData) => {
            ...
            return (
              <LiquidGlass key={h.id} className="h-full flex flex-col p-1 transition-all duration-300 hover:-translate-y-1 hover:shadow-[0_10px_30px_-10px_rgba(0,0,0,0.5)] bg-white/[0.01] border-[var(--border-subtle)] hover:bg-white/[0.02] hover:border-[var(--border-subtle)]">
                <Link href={`/hackathons/${encodeURIComponent(h.name)}/judge`} className="group block">
                  <div className="relative flex flex-col h-full bg-[var(--bg-secondary)] rounded-none p-6 overflow-hidden">
                    ...
                    <div className="flex-1 min-h-0 mb-4">
                      {h.theme && <p className="text-accent/70 text-xs font-mono mb-2 uppercase tracking-wide">{h.theme}</p>}
                      <h2 className="text-xl font-bold text-[var(--text-primary)] mb-2 leading-tight truncate" title={h.name}>{h.name}</h2>
                    ...
                    <div className="mt-auto pt-4 border-t border-[var(--border-subtle)] flex flex-col gap-2">
                      ...
                      {h.maxParticipants && (
                        <div className="flex items-center gap-2 text-xs text-text-muted">
                          ...
                          <span>{h.currentParticipants ?? 0} / {h.maxParticipants} Spots</span>
                        </div>
                      )}
                    </div>
                    <div className="mt-4 flex gap-2">
                      {isRegistered ? (
                        <>
                          <Link href={`/hackathons/${encodeURIComponent(h.name)}/judge`} className="flex-1 px-4 py-2 rounded-none bg-accent/10 border border-accent/20 text-accent text-xs font-bold uppercase tracking-widest hover:bg-accent/20 transition-colors cursor-pointer">
                            Ready to Judge
                          </Link>
                          <Link href={`/hackathons/${encodeURIComponent(h.name)}/participants`} className="px-3 py-2 rounded-none border border-[var(--border-subtle)] text-text-muted hover:text-[var(--text-primary)] hover:bg-white/5 transition-colors">
                        ...
                        <button disabled className="flex-1 px-4 py-2 rounded-none bg-white/5 border border-[var(--border-subtle)] text-gray-600 text-xs font-bold uppercase tracking-widest cursor-not-allowed">
                          Closed
                        </button>
                      )}
                      <Link href={`/hackathons/${encodeURIComponent(h.name)}/participants`} className="px-3 py-2 rounded-none border border-[var(--border-subtle)] text-text-muted hover:text-[var(--text-primary)] hover:bg-white/5 transition-colors">
